...
def metadata(id=None,
  ...
  _update_dependencies(use_orig_data, meta_types_requested)
...
def _get_one(dataset, meta_types, meta_types_requested, update, update_skip, regen,
  ...
  for meta_type in meta_types:
    if meta_types_requested is not None and meta_type not in meta_types_requested:
      # Remove metadata that was generated but not requested.
...
def _cdfmetafile(dataset, update=False, diffs=False, exit_on_exception=False):
  ...
  """
  This function serves the same purpose as _orig_data() in that it provides
  information about the data files associated with a dataset. However,
  ...
  There is some difference in the content returned by _cdfmetafile vs _orig_data.
  In the following example, a * indicates information only returned by _orig_data.
  "FileDescription": [
        {
          "Name": "https://cdaweb.gsfc.nasa.gov/sp_phys/data/ace/orbit/level_2_cdaweb/or_ssc/ac_or_ssc_19970101_v01.cdf",
        * "MimeType": "application/x-cdf",
          "StartTime": "1997-08-25T17:48:00.000Z",
          "EndTime": "1998-01-01T00:00:00.000Z",
        * "Length": 798541,
        * "LastModified": "2017-03-24T04:58:30.491Z"
        },
        ...
    ]
  If needed, this missing information could be obtained with many head requests,
   but at this point it is not needed. In addition, this would probably be slower
  than using the orig_data endpoint.

  To see an example of the difference in content, compare the stdout for
  python metadata.py --id AC_H0_MFI --meta_type orig_data
  with
  python metadata.py --id AC_H0_MFI --meta_type cdfmetafile
  """
  ...
  if need_update:
    ...
    if status_code == 304:
      logger.info(f"Not re-fetching{file} because status_code = 304")
    else:
      if 'emsg' not in info:
        try:
  ...
  # If dataset is None, it means _cdfmetafile is being called to update 
  # cdfmetafiles/info/ files. So we don't return here and instead just
  # update cache files if needed.
  if dataset is not None:
    ...
    if os.path.exists(pkl_file):
      ...
      # cdfmetafile/info/{DATASETID}.pkl
      return cdawmeta.util.read(pkl_file, logger=logger)
    else:
...
def _update_dependencies(use_orig_data, meta_types_requested):
  ...
  for meta_type_ in cdawmeta.dependencies:
    if meta_type_ == 'all':
      continue
    deps = cdawmeta.dependencies[meta_type_]
    if deps is None:
      continue
    if 'cdfmetafile' in deps and meta_type_ in meta_types_requested:
      msg = f"Using orig_data instead of cdfmetafile for {meta_type_} "
      msg += "metadata generation because use_orig_data=True."
      logger.info(msg)
